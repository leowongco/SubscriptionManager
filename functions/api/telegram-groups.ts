...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    try {
        ...
        if (groupId) {
            ...
            if (!group) {
                return new Response('群組不存在', { status: 404 });
            }
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    try {
        ...
        // 验证必填字段
        if (!body.name) {
            return new Response('缺少必填字段: name', { status: 400 });
        }

        if (!body.billing_day || body.billing_day < 1 || body.billing_day > 31) {
            return new Response('扣費日必須在 1-31 之間', { status: 400 });
        }

        if (!body.billing_cycle_type || !['monthly', 'biannually', 'yearly'].includes(body.billing_cycle_type)) {
            return new Response('收費週期必須是 monthly, biannually 或 yearly', { status: 400 });
        }
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
    try {
        ...
        if (!id) {
            return new Response('缺少群組 ID', { status: 400 });
        }
...
export const onRequestDelete: PagesFunction<Env> = async (context) => {
    try {
        ...
        if (subscriptions.length > 0) {
            return new Response(
                `無法刪除群組，仍有 ${subscriptions.length} 個訂閱關聯到此群組。請先解除關聯。`,
                { status: 400 }
            );
        }
        ...
        if (billingCycles.length > 0) {
            return new Response(
                `無法刪除群組，仍有 ${billingCycles.length} 個收費週期關聯到此群組。請先刪除收費週期。`,
                { status: 400 }
            );
        }
