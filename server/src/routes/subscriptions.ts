...
subscriptionsRouter.post('/', (req, res) => {
  ...
  db.prepare(
    'INSERT INTO subscriptions (id, account_id, service_id, start_date, group_name, service_account, next_price, effective_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  ).run(id, body.account_id, body.service_id, startDate, groupName, serviceAccount, nextPrice, effectiveDate);

  logAudit({
    actionType: 'subscription_add',
    entityType: 'subscription',
    entityId: id,
    newValue: { account_id: body.account_id, service_id: body.service_id, service_account: serviceAccount, next_price: nextPrice, effective_date: effectiveDate },
...
subscriptionsRouter.put('/', (req, res) => {
  ...
  db.prepare(
    'UPDATE subscriptions SET group_name = ?, start_date = ?, service_account = ?, next_price = ?, effective_date = ? WHERE id = ?'
  ).run(groupName, startDate, serviceAccount, nextPrice, effectiveDate, id);

  logAudit({
    actionType: 'subscription_update',
    entityType: 'subscription',
    entityId: id,
    oldValue: { group_name: existing.group_name, start_date: existing.start_date, service_account: existing.service_account, next_price: existing.next_price, effective_date: existing.effective_date },
    newValue: { group_name: groupName, start_date: startDate, service_account: serviceAccount, next_price: nextPrice, effective_date: effectiveDate },
