...
* **💬 Telegram 繳費提醒 Bot**：兩種提醒方式可並用——① 幫 Telegram 群組設定 chat_id，未繳費名單會整批發到群組聊天室；② 成員個別綁定自己的 Telegram 後，會收到私訊提醒，可在對話裡按「我已繳費」回報。兩種回報都只是待確認狀態，仍要管理員在後台按「確認收款」才會真的標記為已繳費，避免任何人隨口一按就讓系統誤判收到錢。
...
設定好 `TELEGRAM_BOT_TOKEN` 後：
- **發到群組**（推薦，不用每個人各自綁定）：新增/編輯 Telegram 群組時填「Telegram 群組 Chat ID」——把 bot 加進群組，在群組裡傳一則訊息轉發給 [@userinfobot](https://t.me/userinfobot) 或 [@RawDataBot](https://t.me/RawDataBot) 查詢 chat_id（通常是負數）。設定後未繳費名單會整批發到群組。
- **發給個人**：到「Apple ID 管理」或「訂閱關係對應」頁面，每個成員旁邊會出現一個紙飛機圖示——按下去會產生一個一次性的綁定連結（`https://t.me/你的bot?start=...`），複製傳給該成員本人，對方在 Telegram 點開、按 Start 就完成綁定，之後繳費提醒會私訊發給他。
- 在 Telegram 群組的詳情頁按「管理關聯訂閱」，可以直接勾選哪些 Apple ID / 訂閱屬於這個群組，不用跑去「訂閱關係對應」頁面逐筆設定。
